...
test.describe('Input fields do not accept wrong values', () => {
	test('Values greater than 59 cannot be entered', async ({ page }) => {
		...
		await expect(page.getByRole('spinbutton', { name: 'min' })).toHaveValue('59');
		await expect(page.getByRole('spinbutton', { name: 'sec' })).toHaveValue('59');
	});
	...
	test('Negative values cannot be entered', async ({ page }) => {
		...
		await expect(page.getByRole('spinbutton', { name: 'min' })).toHaveValue('0');
		await expect(page.getByRole('spinbutton', { name: 'sec' })).toHaveValue('0');
	});
});
...
test('Timer starts, pauses and resumes, cancels', async ({ page }) => {
	...
	const minutesAfterPause = await page.getByRole('timer', { name: 'min' }).textContent();
	const secondsAfterPause = await page.getByRole('timer', { name: 'sec' }).textContent();
	expect(minutesAfterPause).toBe('1');
	expect(secondsAfterPause).toBe('0');
	// Timer resumes
	await page.getByRole('button', { name: 'Resume' }).click();
	await page.waitForTimeout(1500);
	const minutesAfterResume = await page.getByRole('timer', { name: 'min' }).textContent();
	const secondsAfterResume = await page.getByRole('timer', { name: 'sec' }).textContent();
